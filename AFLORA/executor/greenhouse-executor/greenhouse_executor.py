...
GREENHOUSE_ACTIONS_JSON_FILE_PATH = "sensors-config/greenhouse-actions.json"
GREENHOUSE_ACTION_TOPIC_STRUCTURE = "/{greenhouse}/{managed_resource}"  # Plant status topic structure
...
def on_message(client, userdata, msg):
    print(f"Greenhouse plan updated {msg.topic}", flush=True)
    regex = r"^\/(.+)\/plan"
    ...
    greenhouse_actions_data = retrieve_actions_data(GREENHOUSE_ACTIONS_JSON_FILE_PATH)
    for managed_resource, action in greenhouse_actions_data[published_plan].items():
        topic = GREENHOUSE_ACTION_TOPIC_STRUCTURE.format(greenhouse=matched_groups[0], managed_resource=managed_resource)
        print(f"Plan: {published_plan}, Topic: {topic}, Action: {action}")
        publish_value_on_topic(client, action, topic)


class GreenhouseExecutor(Executor):
