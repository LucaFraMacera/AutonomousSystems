...
class PlantSprinkler:
    ...
    __OFF_KEY = "OFF"
    __ON_KEY = "ON"
    __FULL_THROTTLE_KEY = "FULL_THROTTLE"
    ...
    def __init__(self, greenhouse_id, plant_id) -> None:
        ...
        self.__status = self.__OFF_KEY

    def turn_on(self, mqtt_client):
        if self.__status != self.__ON_KEY:
            self.__status = self.__ON_KEY
    ...
    def turn_off(self, mqtt_client):
       if self.__status != self.__OFF_KEY:
            self.__status = self.__OFF_KEY
    ...
    def operate_at_full_throttle(self, mqtt_client):
        if self.__status != self.__FULL_THROTTLE_KEY:
            self.__status = self.__FULL_THROTTLE_KEY
